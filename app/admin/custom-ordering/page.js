...
const MEAL_TYPES = [
  { value: 'all', label: 'All Meals', color: 'bg-gray-100 text-gray-700' },
  { value: 'breakfast', label: 'Breakfast', color: 'bg-amber-100 text-amber-700', icon: '🍳' },
  { value: 'lunch', label: 'Lunch', color: 'bg-orange-100 text-orange-700', icon: '🍛' },
  { value: 'dinner', label: 'Dinner', color: 'bg-indigo-100 text-indigo-700', icon: '🍽️' },
];
...
export default function CustomOrderingPage() {
  ...
  const [viewMode, setViewMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('custom_ordering_view_mode') || 'card';
    }
    return 'card';
  });
  ...
  // ─── Meal Settings Functions ───

  const loadMealSettings = async () => {
    try {
      setLoadingSettings(true);
      const response = await api.get('/api/meal-settings');
      if (!response.ok) throw new Error('Failed to load meal settings');
  ...
  const handleToggleMealSetting = async (mealType) => {
    const newSettings = {
      ...mealSettings,
      [`${mealType}_enabled`]: !mealSettings[`${mealType}_enabled`],
    };
    ...
    try {
      ...
      const response = await api.put('/api/meal-settings', newSettings);
      if (!response.ok) {
        // Revert on failure
        setMealSettings(mealSettings);
        throw new Error('Failed to update meal settings');
      }
      const data = await response.json();
      setMealSettings(data.settings);
      toast.success(`${mealType.charAt(0).toUpperCase() + mealType.slice(1)} custom ordering ${newSettings[`${mealType}_enabled`] ? 'enabled' : 'disabled'}`);
    } catch (error) {
      toast.error(error.message);
    } finally {
  ...
  const filteredItems = items.filter((item) => {
    ...
    if (mealTypeFilter !== 'all') {
      if (item.meal_type !== mealTypeFilter && item.meal_type !== 'all') return false;
    }
  ...
  const getMealTypeBadge = (mealType) => {
    const config = MEAL_TYPES.find(m => m.value === mealType) || MEAL_TYPES[0];
    return config;
  };
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">Custom Ordering</h1>
          <p className="text-xs sm:text-sm text-gray-500">Manage à la carte menu for daily basis packages</p>
        </div>
      ...
      {/* Meal Settings Toggles */}
      <div className="bg-gradient-to-r from-primary-50 to-indigo-50 rounded-lg border border-primary-200 p-4 mb-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Custom Ordering Settings</h2>
            <p className="text-xs text-gray-500 mt-0.5">Enable à la carte ordering for daily basis packages</p>
          </div>
          {loadingSettings && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {['breakfast', 'lunch', 'dinner'].map((meal) => {
            const mealConfig = MEAL_TYPES.find(m => m.value === meal);
            const isEnabled = mealSettings[`${meal}_enabled`];
            return (
              <button
                key={meal}
                onClick={() => handleToggleMealSetting(meal)}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-700">{mealConfig.label}</span>
                </div>
        ...
        <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs text-blue-800">When enabled, daily basis users will see a menu to select items when scanning their card instead of auto-generating a token.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3 mb-4">
        <div className="flex flex-col sm:flex-row gap-2 items-center">
          ...
          {(categoryFilter !== 'all' || availabilityFilter !== 'all' || searchQuery || mealTypeFilter !== 'all') && (
            <button
              onClick={() => {
          ...
          <div className="flex border border-gray-300 rounded-lg overflow-hidden">
            <button
              onClick={() => { setViewMode('table'); localStorage.setItem('custom_ordering_view_mode', 'table'); }}
            ...
            <button
              onClick={() => { setViewMode('card'); localStorage.setItem('custom_ordering_view_mode', 'card'); }}
          ...
          <Button variant="outline" size="sm" onClick={() => { loadMealSettings(); loadCategories(); loadItems(); }} className="w-full sm:w-auto">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase">Meal Type</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredItems.map((item, index) => {
                const mealTypeBadge = getMealTypeBadge(item.meal_type);
                return (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-center">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded ${mealTypeBadge.color}`}>
                        <span>{mealTypeBadge.icon}</span>
                        {mealTypeBadge.label}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-center">
            ...
            return (
              <div key={item.id} className={`bg-white rounded-lg border overflow-hidden transition-shadow hover:shadow-md ${!item.is_available ? 'border-gray-200 opacity-75' : 'border-gray-200'}`}>
                ...
                <div className="relative aspect-[4/3] bg-gray-100">
                  ...
                  {/* Meal Type badge */}
                  <span className={`absolute bottom-2 left-2 px-2 py-0.5 text-[11px] font-medium rounded-full shadow-sm backdrop-blur-sm ${mealTypeBadge.color} bg-opacity-95`}>
                    {mealTypeBadge.icon} {mealTypeBadge.label}
                  </span>
                  {/* Availability badge */}
                  <button
                    onClick={() => handleToggleAvailability(item)}
        ...
        <form onSubmit={handleItemSubmit}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Select which meal time this item is available for. "All Meals" shows it for all meal times.</p>
            </div>

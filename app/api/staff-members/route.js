...
const sanitizeData = (data) => {
  ...
  const optionalFields = ['medical_dietary_requirements', 'special_requirements', 'notes', 'photo_url', 'food_allergies_medical_needs', 'emergency_contact_name', 'emergency_contact_number', 'fee_payment_other_details'];
...
export async function GET(request) {
  try {
    ...
    if (search) {
      query = query.or(`full_name.ilike.%${search}%,employee_id.ilike.%${search}%,cnic_no.ilike.%${search}%,membership_id.ilike.%${search}%`);
    }
